...
_ROMAN_TRANSLATE = str.maketrans({"?":"i","?":"i","?":"x","?":"x","?":"c","?":"c","?":"v","?":"v","?":"m","?":"m","?":"d","?":"d","?":"l","?":"l"})
_ROMAN_TOKEN_RE  = re.compile(r"\b[ivxlcdm???????]+\b", re.I)
_LEADING_SECTION = re.compile(r"^\s*((?:[ivxlcdm???????]+|[0-9]+))\.?\s+", re.I)  # strip leading I./??./1. etc.
...
def is_alignment_row(cells: List[str]) -> bool:
    return all(re.fullmatch(r"\s*:?-{3,}:?\s*", (c or "")) for c in cells if cells is not None)
    ...
    for ln in lines:
        if ln.strip().startswith("|") and ln.strip().endswith("|"):
            ...
            if cur:
                ...
                cur = []
    if cur and len(cur) >= 2: tables.append(cur)
    cleaned = []
    for tbl in tables:
        cleaned.append([tbl[0]] + tbl[2:] if len(tbl) >= 2 and is_alignment_row(tbl[1]) else tbl)
    return cleaned
...
def extract_code_label_numbers_from_row(row: List[str]) -> Tuple[Optional[str], str, Optional[int], Optional[int]]:
...
def extract_code_map_and_label_rows(tables: List[List[List[str]]]) -> Tuple[
        ...
        for row in tbl:
            ...
            if code:
                if code not in code_map:
                    ...
                    prev = code_map[code]
                    prev_n = (prev.get("start") is not None) + (prev.get("end") is not None)
                ...
                if norm:
                    label_rows_nocode.append({"label": label, "start": start, "end": end, "norm": norm})
...
def jaccard_char_sim(a: str, b: str) -> float:
    ...
    sa = set(na.split()); sb = set(nb.split())
...
def num_similarity(gs: Optional[int], rs: Optional[int], ge: Optional[int], re_: Optional[int]) -> Optional[float]:
    s_rel = rel_error(gs, rs); e_rel = rel_error(ge, re_)
...
# ---------- formatting ----------
def format_numbers_with_code(code: Optional[str], start: Optional[int], end: Optional[int], side_has_code: bool) -> str:
    if not side_has_code: return ""
    parts = [code] if code else []
    if start is not None: parts.append(str(start))
    if end   is not None: parts.append(str(end))
    return " ".join(parts)
...
def match_label_only_rows(gt_rows: List[Dict[str, Any]], res_rows: List[Dict[str, Any]], sim_thresh: float) -> List[Tuple[int,int,float]]:
    CAND_TJACCARD = 0.50
    CAND_SHARED   = 2
    scored = []
    for i, g in enumerate(gt_rows):
        for j, r in enumerate(res_rows):
            ...
            shared = len(tokenize_meaningful(g["label"]) & tokenize_meaningful(r["label"]))
    ...
    for i, j, sim in scored:
        if i in matched_gt or j in matched_res: continue
        matched_gt.add(i); matched_res.add(j); out.append((i, j, sim))
    return out
...
def build_dataframe(gt_path: Path, res_path: Path) -> pd.DataFrame:
    ...
    for cg in rem_gt_codes:
        g = gt_map[cg]; glabel, gs, ge = g["label"], g["start"], g["end"]
        best_code, best_sim = None, -1.0
        for cr in rem_res_codes:
            if cr in matched_res_codes: continue
            sim = jaccard_char_sim(glabel, res_map[cr]["label"])
            if sim > best_sim: best_code, best_sim = cr, sim
        if best_code is not None and best_sim >= CROSS_CODE_SIM_THRESH:
            r = res_map[best_code]; rs, re_ = r["start"], r["end"]
            lab_sim = best_sim; n_sim = num_similarity_with_codes(gs, rs, ge, re_, True, True)
            s_rel, e_rel = rel_error(gs, rs), rel_error(ge, re_)
            rows.append({
                "gt_label": glabel, "res_label": r["label"],
            ...
            matched_res_codes.add(best_code); used_gt_codes.add(cg)
    ...
    for cg in [c for c in gt_map.keys() - used_gt_codes]:
        ...
        for j, r in enumerate(res_nocode):
            if j in matched_res_nc: continue
            ...
            if sim > best_sim and (sim >= CROSS_CODE_SIM_THRESH or tj >= 0.50 or shared >= 2):
                best_j, best_sim = j, sim
        if best_j is not None:
            r = res_nocode[best_j]; rs, re_ = r["start"], r["end"]
            lab_sim = best_sim
            ...
            matched_res_nc.add(best_j); used_gt_codes.add(cg)

    #    b) RES code ↔ GT no-code
    matched_gt_nc = set()
    for cr in [c for c in res_map.keys() - used_res_codes]:
        r = res_map[cr]; rs, re_ = r["start"], r["end"]; rlabel = r["label"]
        best_i, best_sim = None, -1.0
        for i, g in enumerate(gt_nocode):
            if i in matched_gt_nc: continue
        ...
        if best_i is not None:
            g = gt_nocode[best_i]; gs, ge = g["start"], g["end"]
            lab_sim = best_sim; n_sim = num_similarity_with_codes(gs, rs, ge, re_, False, True)
            s_rel, e_rel = rel_error(gs, rs), rel_error(ge, re_)
            rows.append({
                "gt_label": g["label"], "res_label": rlabel,
            ...
            matched_gt_nc.add(best_i); used_res_codes.add(cr)

    # 4) One-sided leftovers WITH CODE
    for cg in gt_map.keys() - used_gt_codes:
        g = gt_map[cg]; glabel = g["label"]; gs, ge = g["start"], g["end"]
        rows.append({"gt_label": glabel, "res_label": "",
                     "gt_numbers": format_numbers_with_code(cg, gs, ge, True), "res_numbers": "",
                     "label_sim": np.nan, "num_sim": None, "overall_sim": None,
                     "start_rel_diff_%": None, "end_rel_diff_%": None})
    for cr in res_map.keys() - used_res_codes:
        r = res_map[cr]; rs, re_ = r["start"], r["end"]
        rows.append({"gt_label": "", "res_label": r["label"],
                     "gt_numbers": "", "res_numbers": format_numbers_with_code(cr, rs, re_, True),
                     "label_sim": np.nan, "num_sim": None, "overall_sim": None,
                     "start_rel_diff_%": None, "end_rel_diff_%": None})

    # 5) No-code↔no-code label-only matching
    matches = match_label_only_rows(gt_nocode, res_nocode, sim_thresh=LABEL_ONLY_SIM_THRESH)
    ...
    for i, j, sim in matches:
        ...
        s_rel, e_rel = rel_error(gs, rs), rel_error(ge, re_)
        rows.append({
            "gt_label": g["label"], "res_label": r["label"],
            "gt_numbers": format_numbers_no_code(gs, ge),
            "res_numbers": format_numbers_no_code(rs, re_),
            "label_sim": round(sim,4), "num_sim": n_sim,
            "overall_sim": round(float(np.nanmean([sim, n_sim if n_sim is not None else np.nan])), 4),
            "start_rel_diff_%": None if s_rel is None else round(100*s_rel,4),
            "end_rel_diff_%":   None if e_rel is None else round(100*e_rel,4),
        })
    ...
    for idx, g in enumerate(gt_nocode):
        if idx in matched_gt_nc2: continue
        rows.append({"gt_label": g["label"], "res_label": "",
                     "gt_numbers": format_numbers_no_code(g["start"], g["end"]), "res_numbers": "",
                     "label_sim": np.nan, "num_sim": None, "overall_sim": None,
                     "start_rel_diff_%": None, "end_rel_diff_%": None})
    for idx, r in enumerate(res_nocode):
        if idx in matched_res_nc2: continue
        rows.append({"gt_label": "", "res_label": r["label"],
                     "gt_numbers": "", "res_numbers": format_numbers_no_code(r["start"], r["end"]),
                     "label_sim": np.nan, "num_sim": None, "overall_sim": None,
                     "start_rel_diff_%": None, "end_rel_diff_%": None})

    df = pd.DataFrame(rows)
    # sort: paired first (both labels present), then one-sided
    df["__both__"] = (df["gt_label"].str.len().fillna(0) > 0) & (df["res_label"].str.len().fillna(0) > 0)
    df = df.sort_values(by=["__both__", "gt_label", "res_label"], ascending=[False, True, True]).drop(columns="__both__")
    front = ["gt_label","res_label","gt_numbers","res_numbers","label_sim","num_sim","overall_sim"]
    return df[front + [c for c in df.columns if c not in front]]


# ---------- HTML ----------
def write_html(df: pd.DataFrame, out_html: Path, gt_path: Path, res_path: Path):
    avg_label = float(df["label_sim"].dropna().mean()) if df["label_sim"].notna().any() else np.nan
    avg_num   = float(df["num_sim"].dropna().mean())   if df["num_sim"].notna().any()   else np.nan
    avg_over  = float(df["overall_sim"].dropna().mean()) if df["overall_sim"].notna().any() else np.nan
    ...
    def filled_num_display(num_sim_val, label_sim_val):
        is_missing = (num_sim_val is None) or (isinstance(num_sim_val, float) and np.isnan(num_sim_val))
        if not is_missing:
            return num_sim_val, False
        if NUM_DISPLAY_FILL == "mirror_label":
            return (label_sim_val if not (label_sim_val is None or (isinstance(label_sim_val, float) and np.isnan(label_sim_val))) else 0.0), True
    ...
    for _, row in df.iterrows():
        ...
        badge = "<span class='badge' title='display fill; not used in averages'>•</span>" if filled else ""
        html_rows.append(
            "<tr>"
...
def pair_files(gt_files: List[Path], res_files: List[Path]) -> List[Tuple[Path, Path, str]]:
    ...
    for key, g in list(gt_map.items()):
        if key in res_map and res_map[key] in res_unused:
            ...
            del gt_map[key]; del res_map[key]
    ...
    for score, g, r in scored:
        if g in used_g or r in used_r: continue
        if (score >= FUZZY_MIN_RATIO) or (_token_jaccard_name(g, r) >= FUZZY_TOKEN_MIN_JACC):
            pairs.append((g, r, f"fuzzy:{score:.3f}"))
            used_g.add(g); used_r.add(r)
            if r in res_unused: res_unused.remove(r)
...
def main():
    ...
    for idx, (gtp, resp, reason) in enumerate(pairs, start=1):
        ...
        uniq = hashlib.md5((gtp.name + "||" + resp.name).encode("utf-8", "ignore")).hexdigest()[:8]
        ...
        print(f"     -> {csv_path}")
        print(f"     -> {html_path}")
        print(f"     Avg: overall={avg_over:.4f} | label={avg_label:.4f} | num={avg_num:.4f}")
    ...
    if index_rows:
        ...
        try: webbrowser.open(idx_html.as_uri())
